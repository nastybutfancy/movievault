  ...
  return new Promise(function (resolve, reject) {
    ...
    window[callbackName] = function (data) {
      ...
      if (data && data.success === false) {
        reject(
          new Error(
            data.message || "Wystąpił błąd API."
          )
        );
      ...
      await codeReader.decodeFromConstraints(
        ...
        function (result) {
          if (result && !scanLock) {
            ...
            const barcode =
              normalizeBarcode(result.getText());
...
function renderScanResult(barcode, movie) {
  $("resultPanel").classList.remove("hidden");
  ...
  if (movie) {
    ...
    $("resultPanel").innerHTML = `
      ...
      Rok:
      ${escapeHtml(movie.year || "brak")}
      <br>

      Półka:
      ${escapeHtml(movie.shelf || "brak")}
      <br>
...
async function addMovie() {
  ...
  try {
    ...
    button.textContent = "Zapisz film";
  }
}
